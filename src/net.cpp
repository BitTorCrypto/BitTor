...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BitTor " + FormatFullVersion();
...
static const char *strMainNetOnionSeed[][1] = {
    {"sow7ki423toi4rrz.onion"},
	{"wd7ytfordbjulbyp.onion"},
    {NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitTor is probably already running."), addrBind.ToString().c_str());
        else
