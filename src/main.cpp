...
uint256 const hashGenesisBlock("0x00000abd423af69eb99801cf04167de8bf2278187d82c9a3a76c986452a0794b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitTor: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitTor Signed Message:\n";
...
{
    // BitTor: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 2 * 60; // 2 minutes blocks
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 7.5 blocks
...
{
        static const int64 BlocksTargetSpacing = 4 * 60; // 4 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitTor: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BitTor: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Anonymous. Anonymous. Anonymous.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406503059;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1511270;
        ...
        assert(block.hashMerkleRoot == uint256("0x9eade33fd582dcf91deda1ea2a7fae53293f2cf9f9bcfb1e8df7b18d5972c559"));
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb9, 0xaa, 0x7f }; // BitTor: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitTorMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitTorMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitTorMiner(CWallet *pwallet)
{
    printf("BitTorMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bittor-miner");
    ...
    try { loop {
        ...
        printf("Running BitTorMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitTorMiner, pwallet));
}
